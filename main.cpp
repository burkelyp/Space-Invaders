...
int main(int argc, char** argv) {
    ...
    while (running) {
        ...
        if (!paused) {
            while (cycles_for_interrupt_timing < CYCLES_PER_INTERRUPT) {
                uint64_t before = state.cycles;
                Emulate8080Op(&state);
                cycles_for_interrupt_timing += (state.cycles - before);
            }
        }
        ...
        if (cycles_for_interrupt_timing >= CYCLES_PER_INTERRUPT) {
            ...
            if (state.int_enable) {
                /**
                * Simulate hardware interrupt by calling RST 1 or RST 2 alternately.
                * Each interrupt pushes the PC and jumps to an appropriate vector.
                */
